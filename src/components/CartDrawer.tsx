...
export function CartDrawer() {
  ...
  // Escape closes the drawer; suppressed while checkout is stacked on top so
  // Escape only dismisses the innermost overlay.
  useEffect(() => {
    if (!isCartOpen) return;
    const onKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Escape" && !isCheckoutOpen) toggleCart();
    };
    window.addEventListener("keydown", onKeyDown);
    return () => window.removeEventListener("keydown", onKeyDown);
  }, [isCartOpen, isCheckoutOpen, toggleCart]);
  ...
  return (
    <>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                {cartItems.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center space-y-4 text-center">
                    ...
                    <button 
                      onClick={toggleCart}
                      className="bg-primary text-white px-8 py-3 rounded-sm text-xs font-bold uppercase tracking-widest hover:bg-primary/90 transition-all"
                    >
                      Start Shopping
                    </button>
                  </div>
                  ...
                  cartItems.map((item) => (
                    ...
                    >
                      ...
                      <div className="flex-1 flex flex-col">
                        ...
                        <p className="text-sm text-gray-500 mt-1">{formatMoney(item.price)}</p>
                        ...
                        <div className="mt-auto flex items-center justify-between">
                          <div className="flex items-center border border-gray-200 dark:border-gray-700 rounded-sm">
                            ...
                            <button
                              ...
                              className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors dark:text-gray-400 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary rounded-sm disabled:opacity-40 disabled:cursor-not-allowed"
                              aria-label="Increase quantity"
                            >
                              <Plus size={14} />
                            </button>
                          </div>
                          <p className="text-sm font-semibold dark:text-gray-200">{formatMoney(item.price * item.quantity)}</p>
                        </div>
                      </div>
                    </motion.div>
                  ))
              ...
              {cartItems.length > 0 && (
                <div className="p-6 border-t border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/50 backdrop-blur-md">
                  <div className="flex justify-between items-center mb-6">
                    ...
                    <span className="text-xl font-semibold dark:text-gray-100">{formatMoney(cartTotal)}</span>
                  </div>
                  <button onClick={() => setIsCheckoutOpen(true)} className="w-full bg-primary text-white py-4 rounded-sm text-sm font-bold uppercase tracking-[0.2em] hover:bg-primary/90 transition-all shadow-lg hover:shadow-xl active:scale-[0.98] ">
                    Checkout Now
                  </button>
      ...
      {/* Mounted outside the drawer's AnimatePresence so the modal's own
          exit animation is never cut short by the drawer unmounting. */}
